...
describe('Subscription API', () => {
    afterAll(async () => {
        if (app && app.server) {
            ...
            console.warn(
                'Server instance not found on app, cannot close explicitly. This might cause Jest to hang.'
            );
        }
    });
    ...
    it('should not allow duplicate emails', async () => {
        ...
        const firstResponse = await request(app).post('/api/subscribe').send({
            email,
            city: 'Kharkiv',
            frequency: 'daily',
        });
        ...
        const secondResponse = await request(app).post('/api/subscribe').send({
            email,
            city: 'Kharkiv',
            frequency: 'daily',
        });
