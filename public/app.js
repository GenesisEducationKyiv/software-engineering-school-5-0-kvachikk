...
function showMessage(e,t,r="info"){
    ...
    "success"===r?e.classList.add("message-success"):"error"===r?e.classList.add("message-error"):"loading"===r&&e.classList.add("message-loading");
...
function createWeatherCardHTML(e,t){
    return !t || void 0 === t.temperature ? `<div class="weather-card-item p-6 text-center"><h3 class="text-xl font-semibold mb-2">${e.charAt(0).toUpperCase()+e.slice(1)}</h3><p class="text-red-500">Could not load weather data.</p></div>` : `
...
function createForecastCardHTML(e,t){
    if(!t||0===t.length)return`<div class="weather-card-item p-6 text-center"><h3 class="text-xl font-semibold mb-2">${e.charAt(0).toUpperCase()+e.slice(1)} - Forecast</h3><p class="text-red-500">Could not load forecast data.</p></div>`;const r=t.map(((t,r)=>{let l="";if(0===r)l="Today";else if(1===r)l="Tomorrow";else{const d=new Date;d.setDate(d.getDate()+r);l=d.toLocaleDateString(void 0,{weekday:"short",day:"numeric",month:"short"})}return`\n                            <div class="weather-card-item p-4 md:p-5 flex flex-col items-center justify-center space-y-1">\n                                <h4 class="text-md font-semibold">${l}</h4>\n                                <p class="text-3xl font-bold">${t.temperature?.toFixed(1)}°C</p>\n                                <p class="text-sm text-blue-500 capitalize">${t.description||"N/A"}</p>\n                                <p class="text-sm">Humidity: ${t.humidity||"N/A"}%</p>\n                            </div>`})).join("");return`\n                    <div class="p-4 md:p-5">\n                        <h3 class="text-xl lg:text-2xl font-bold mb-4">${e.charAt(0).toUpperCase()+e.slice(1)} - 4-Day Forecast</h3>\n                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">${r}</div>\n                    </div>`}

async function fetchAndDisplayWeather(e,t,r,s=!1){
    r && showMessage(r,`Loading weather for ${e}...`,"loading");
    let a;
    "keyCitiesGrid"===t.id ? (a=document.createElement("div"),s&&a.classList.add("my-location-highlight"),a.classList.add("weather-card-wrapper"),s?keyCitiesGrid.prepend(a):keyCitiesGrid.appendChild(a)) : (a=t,a.innerHTML="");
    try {
...
async function fetchAndDisplayForecast(e,t,r){
    r && showMessage(r,`Loading forecast for ${e}...`,"loading"),
    t.innerHTML="";
    try {
...
getWeatherBtn.addEventListener("click",(()=>{
    ...
    return e ? (void fetchAndDisplayWeather(e,searchWeatherResultCardWrapper,searchWeatherMessageDiv)) : (showMessage(searchWeatherMessageDiv,"Please enter a city name.","error"),searchWeatherResultCardWrapper.classList.remove("visible"),void(searchWeatherResultCardWrapper.innerHTML=""))
}));
...
subscriptionForm.addEventListener("submit",(async e=>{
    ...
    try {
        const e = await fetch(SUBSCRIBE_API_URL,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:a.toString()}),
              t = await e.json();
        if(200===e.status || 201===e.status) showMessage(subscriptionMessageDiv,t.message||"Subscription successful. Confirmation email sent.","success"),subscriptionForm.reset();
        else {
            let r = `Error ${e.status}: `;
            r += t.errors && Array.isArray(t.errors) ? t.errors.map((e=>e.msg)).join(", ") : t.message || "Failed to subscribe.";
...
getForecastBtn.addEventListener("click",(()=>{
    ...
    return e ? (void fetchAndDisplayForecast(e,forecastWeatherResultWrapper,forecastWeatherMessageDiv)) : (showMessage(forecastWeatherMessageDiv,"Please enter a city name.","error"),forecastWeatherResultWrapper.classList.remove("visible"),void(forecastWeatherResultWrapper.innerHTML=""))
}));
...
async function initializePage(){
    ...
    if(e) await fetchAndDisplayWeather(e,keyCitiesGrid,keyCitiesMessageDiv,!0);
        ...
        e.innerHTML = createWeatherCardHTML("My Location",null).replace("Could not load weather data.","Location not found. Search manually."),
